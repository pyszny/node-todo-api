...
app.get('/todos/:id', authenticate, (req, res) => {                       // setting route for GET method
...
app.delete('/todos/:id', authenticate, (req, res) => {
    let id = req.params.id;                                     // req.params - storage for url parameters
...
app.post('/users/login', (req, res) => {
    ...
    User.findByCredentials(body.email, body.password).then((user) => {
        user.genereteAuthToken().then((token) => {
            res.header('x-auth', token).send(user);
        });
    }).catch((e) => {
        res.status(400).send();
